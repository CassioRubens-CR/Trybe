...
// Logo, adicione na função sendMarsTemperature um outro callback
...
const sendMarsTemperature = (callback, ErrorGettingTemperature) => {
  const marsTemperature = getMarsTemperature();
  const successRate = Math.random() <= 0.6;
  setTimeout(() => {
    if (successRate) callback(marsTemperature)
    else ErrorGettingTemperature('Robot is busy');
  }, messageDelay());
}
